...
vc4_print_insn (CGEN_CPU_DESC cd, bfd_vma pc, disassemble_info *info)
{
  ...
  bool symbol_here = (*info->symbol_at_address_func) (pc, info);
...
/* Vector insn printing.
   ...
   AREG_P selects the "extended A operand" coordinate layout, in which the
   four extra bits at 19..16 of the canonical value supply the fine
   coordinate of the axis the 6-bit field does not carry.  That layout is a
   property of the FIELD, not of the operand's role: only the 80-bit A
   specifier has those extra bits (f-vec80areg's f-op51-48 subfield).  The
   48-bit A specifier is a plain 9-bit field shaped exactly like the 48-bit D
   and B ones, so it must be printed with AREG_P false -- see
   print_vec48aluareg_v.  */

static void
print_vector_reg_1 (void *dis_info, unsigned long value, vc4_operand whichop,
                    bool areg_p)
{
  vc4_vec_dir vec_dir;
  unsigned x, y;
  bool do_inc, column_offset;
  unsigned scalar_reg;
  disassemble_info *info = (disassemble_info *) dis_info;
  const char *typename;
...
{
  print_vector_reg_1 (dis_info, value, whichop, whichop == OP_A);
}
...
/* The 48-bit A operand is a 9-bit specifier (f-vec48areg: f-op21-19,
   f-op17-16, f-op47-44), laid out exactly like the 48-bit D and B ones -- six
   coordinate bits at 5..0 and three selector bits at 9..7, with no gap-6 and
   no extra nibble at 19..16.  It therefore must NOT be printed with the
   extended-A layout, which belongs to the 20-bit f-vec80areg.

   Getting that wrong was invisible in the H direction, where the two layouts
   coincide once the absent extra bits read as zero, and wrong in the V
   direction, where the extended layout reads all six coordinate bits as the
   row and drops the column's low nibble.  It printed rows that cannot exist:
   a V operand covers sixteen consecutive rows from a 16-aligned base (probed
   at y = 0, 16 and 48, vpu-sem/gen-vdir-probes.py), so only two row bits are
   encodable, yet `V(63,0)` and `V(53,0)` were being printed.  What those
   really are is `V(48,15)` and `V(48,5)`, which is also what the SLEIGH spec
   decodes them to (vc4.slaspec's vay/vax under `Vopsize=0 & Vdir=1`:
   row = VaHi << 4, column = VaLo + 16 * base).  The assembler's parse path
   had it right all along, which is why V-direction disassembly did not
   re-assemble.  */
...
print_vec48aluareg_h (CGEN_CPU_DESC cd ATTRIBUTE_UNUSED,
	              ...
	              int length ATTRIBUTE_UNUSED)
{
  print_vector_reg_1 (dis_info, (value & ~0x40) | 0xf000, OP_A, false);
}
...
print_vec48aluareg_v (CGEN_CPU_DESC cd ATTRIBUTE_UNUSED,
	              ...
	              int length ATTRIBUTE_UNUSED)
{
  print_vector_reg_1 (dis_info, value | 0xf040, OP_A, false);
}
