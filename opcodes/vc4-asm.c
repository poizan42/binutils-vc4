...
/* True if the operand text at *STRP is a bare `-' with nothing after it
   that could be an expression.  Gas's expression parser would accept that
   with only a warning ("Unary operator - ignored...") and assemble garbage,
   so immediate parsers must refuse it outright.  This matters for the
   B slot of the vector insns: the register forms reject `-' there (it has
   no absent encoding), and without this check the immediate variants would
   silently pick the bare `-' up instead.  */

static bool
bare_dash_p (const char *ptr)
{
  if (*ptr != '-')
    return false;
  ptr++;
		    ...
		    unsigned long *valuep, unsigned bits, unsigned shift)
{
  ...
  if (bare_dash_p (*strp))
    return "missing immediate (bare `-' is not an operand)";
                  ...
                  bool extended)
{
  ...
  bool areg_p = (whichop == OP_A) && extended;
  ...
  if ((ptr[0] == 'H' && ptr[1] == 'X')
  ...
  else if (ptr[0] == '-')
    {
      /* This is a special case.  Just hard-wire the "dash" encoding and
         return.  Except in the B slot: there is no absent encoding for it
         -- 0x380 is selector 7/14 plus register 0, so a bare `-' would
         silently mean "r0".  The operand is semantically live (it supplies
         e.g. vgetacc's shift amount), so refuse the spelling and make the
         register explicit.  */
      if (whichop == OP_B)
  ...
  while (**strp != 0)
    {
      ...
      /* This one must come before other "IFN*" predicate checks.  */
                   ...
                   || strncmp ((*strp) + 1, "SUB", 3) == 0))
        {
          ...
          /* SUB_BIT is what makes this group a SUBTRACT rather than an add; without it every
             UDEC/SDEC/USUB/SSUB assembled to the encoding of its UACC/SACC/UADD/SADD counterpart,
             which the disassembler then printed back under the wrong name.  The printer's own
             table below pairs WBA_BIT|SUB_BIT with "UDEC", so the two directions disagreed.  */
          acc_sru |= ENA_BIT | SUB_BIT;
			   ...
			   CGEN_FIELDS * fields)
{
  ...
  switch (opindex)
    ...
    case VC4_OPERAND_ALU32ABREG :
      errmsg = cgen_parse_keyword (cd, strp, & vc4_cgen_opval_h_reg, & fields->f_alu32abreg);
      break;
